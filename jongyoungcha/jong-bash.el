...
(defun jong-bash-bashdb-next ()
  ...
  (let ((prev-line)
    ...
    (with-current-buffer (get-buffer jong-bash-bashdb-buffer)
      ...
      (setq line-content (thing-at-point 'line))
      (call-interactively 'end-of-buffer)
      
      (message "%s" line-content)
      (string-match "^\(\\([...]\\)\):" line-content)
      
      (message (match-string 2 line-content))
      ;; (find-file line-content)
      
      ;; (call-interactively 'end-of-buffer)
      ;; (line-beginning-position )
      ;; (setq next-line (line-number-at-pos))
      ;; (goto-line next-line (current-buffer)
      ))
  )
